    ...
    >
      <div className={css.logoRow}>
        {/* Expanded, the brand doubles as a New Session shortcut; the
            collapsed rail keeps only the expand toggle below. */}
        {wide && (
          <button
            type="button"
          ...
          >
            <span className={css.brandIdentity} aria-hidden="true">
              {/* The mark rides the slot's own display:contents wrapper: an
                  unoccupied slot generates no flex item, so the identity row's
                  gap stays off the name. */}
              {renderSlot('sidebar.brand.mark', { size: 24 })}
              <span className={css.brandName}>
                {renderSlot('sidebar.brand.name', {}, {
                  fallback: (
                    <>
                      <span className={css.fallbackBrandName}>Kliping</span>
