...
展开的品牌行把 `sidebar.brand.mark` 与 `sidebar.brand.name` 渲染为两个独立的 single slot，收起轨道则只保留面板切换控件。没有占位者时，mark 不渲染任何内容，name 是带有构建期 7 位 `DSH_CLIENT_COMMIT_HASH` 徽标的 `Kliping` 标签。部署包可以单独占用任一 slot，而无须替换 New Session 控件或轨道几何；声明感知的 `slots.inject()` 让这种包无论先于还是后于侧边栏激活都能生效。
